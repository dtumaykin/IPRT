...
bool BMP::LoadFile( const char* FileName )
{
 using namespace std;
 if( !EasyBMPcheckDataSize() )
 {
...
bool BMP::WritePixelToFile(int x, int y)
{
	ebmpBYTE RedBYTE = (ebmpBYTE) (Pixels[x][y]).Red;
	ebmpBYTE GreenBYTE = (ebmpBYTE) (Pixels[x][y]).Green;
	ebmpBYTE BlueBYTE = (ebmpBYTE) (Pixels[x][y]).Blue;

	fseek(BMPFile,BMPOffset+3*x+(Height-(y+1))*(3*Width+(4-(3*Width)%4)%4),SEEK_SET);
    fwrite( (char*) &BlueBYTE , 1, 1, BMPFile);
    fwrite( (char*) &GreenBYTE , 1, 1, BMPFile);
    fwrite( (char*) &RedBYTE , 1, 1, BMPFile);

	return true;
}

bool BMP::WritePixelToPaper(int x, int y)
{
    ebmpWORD TempWORD;
	
	ebmpWORD RedWORD = (ebmpWORD) ((Pixels[x][y]).Red / 8);
	ebmpWORD GreenWORD = (ebmpWORD) ((Pixels[x][y]).Green / 4);
	ebmpWORD BlueWORD = (ebmpWORD) ((Pixels[x][y]).Blue / 8);
	
    TempWORD = (RedWORD<<11) + (GreenWORD<<5) + BlueWORD;
	if( IsBigEndian() )
	{ TempWORD = FlipWORD( TempWORD ); }

	fseek(BMPFile,BMPOffset+3*x+(Height-(y-1))*(3*x+(4-3*x%4)%4),SEEK_SET);
    fwrite( (char*) &TempWORD , 2, 1, BMPFile);

	return true;
}

bool BMP::WriteFileHeader()
{
 ...
 bmfh.bfReserved1 = 0; 
 bmfh.bfReserved2 = 0; 
 BMPOffset = bmfh.bfOffBits = (ebmpDWORD) (14+40+dPaletteSize);  
 
 if( IsBigEndian() )
 { bmfh.SwitchEndianess(); }
 
 fwrite( (char*) &(bmfh.bfType) , sizeof(ebmpWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmfh.bfSize) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmfh.bfReserved1) , sizeof(ebmpWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmfh.bfReserved2) , sizeof(ebmpWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmfh.bfOffBits) , sizeof(ebmpDWORD) , 1 , BMPFile );
 ...
 fwrite( (char*) &(bmih.biSize) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biWidth) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biHeight) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biPlanes) , sizeof(ebmpWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biBitCount) , sizeof(ebmpWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biCompression) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biSizeImage) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biXPelsPerMeter) , sizeof(ebmpDWORD) , 1 , BMPFile );
 fwrite( (char*) &(bmih.biYPelsPerMeter) , sizeof(ebmpDWORD) , 1 , BMPFile ); 
 fwrite( (char*) &(bmih.biClrUsed) , sizeof(ebmpDWORD) , 1 , BMPFile);
 fwrite( (char*) &(bmih.biClrImportant) , sizeof(ebmpDWORD) , 1 , BMPFile);
 ...
 if( BitDepth == 1 || BitDepth == 4 || BitDepth == 8 )
 {
  ...
  int n;
  for( n=0 ; n < NumberOfColors ; n++ )
  { fwrite( (char*) &(Colors[n]) , 4 , 1 , BMPFile ); }
 }
// fclose(fp);
 return true;
}

bool BMP::WritePaperHeader()
{
 ...
 {  
  ...
  while( j > -1 )
  {
   ...
   if( Success )
   {
    int BytesWritten = (int) fwrite( (char*) Buffer, 1, BufferSize, BMPFile );
    if( BytesWritten != BufferSize )
    { Success = false; }
   }
   if( !Success )
   {
    if( EasyBMPwarnings )
    {
		std::cout << "EasyBMP Error: Could not write proper amount of data." << std::endl;
	}
    j = -1; 
   }
   j--; 
 ...
 if( BitDepth == 16 )
 {
  ...
  if( IsBigEndian() )
  { RedMask = FlipWORD( RedMask ); }
  fwrite( (char*) &RedMask , 2 , 1 , BMPFile );
  fwrite( (char*) &ZeroWORD , 2 , 1 , BMPFile );

  if( IsBigEndian() )
  { GreenMask = FlipWORD( GreenMask ); }
  fwrite( (char*) &GreenMask , 2 , 1 , BMPFile );
  fwrite( (char*) &ZeroWORD , 2 , 1 , BMPFile );

  if( IsBigEndian() )
  { BlueMask = FlipWORD( BlueMask ); }
  fwrite( (char*) &BlueMask , 2 , 1 , BMPFile );
  fwrite( (char*) &ZeroWORD , 2 , 1 , BMPFile );
